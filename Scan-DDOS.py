...
# --- Umbrales para Detección de DoS (ajusta estos valores) ---
UMBRAL_PAQUETES_DOS = 100
VENTANA_TIEMPO_DOS = 10  # segundos
trafico_reciente = defaultdict(lambda: defaultdict(lambda: []))  # origen -> destino -> [timestamps]
...
def obtener_ubicacion(ip_address):
    # (función obtener_ubicacion sin cambios)
    try:
        if lector_geoip:
...
def mostrar_paquete(paquete):
    ...
    if IP in paquete:
        ...
        ubicacion_destino = obtener_ubicacion(ip_destino)
        print(f"IP ({protocolo_nombre}): {ip_origen} -> {ip_destino} ({ubicacion_destino})")
        detectar_dos(ip_origen, ip_destino)

        if TCP in paquete and paquete[TCP].flags == 'S':
            timestamp = time.time()
